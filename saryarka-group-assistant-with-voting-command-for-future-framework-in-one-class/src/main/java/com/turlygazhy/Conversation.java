...
public class Conversation {
    ...
    public void handleUpdate(Update update, Bot bot) throws SQLException, TelegramApiException {
        ...
        try {
            ...
            if (command == null) {
                ...
                bot.sendMessage(new SendMessage().setChatId(update.getMessage().getChatId()).setText("Тестовая клавиатура")
                .setReplyMarkup(testKeys()));
                return;
            }
        }
    ...
    private InlineKeyboardMarkup testKeys(){
        ArrayList<String> names = new ArrayList<>();
        names.add("результаты голосования");
        names.add("пирог");
        names.add("разослать тем кто голосовал");
        names.add("мои голосования");
        ArrayList<String> data = new ArrayList<>();
        data.add("getVoteResults:25");
        data.add("getVoteGraphs:25");
        data.add("makeResMailing:25");
        data.add("getMyVotings");
        return InlineKeyboardConstructor.getKeyboard(names,data);
    }
}
